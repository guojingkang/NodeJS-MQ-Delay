...
class Delay {
  ...
  async _init() {
    const mq = this._mq;
    await mq.assertExchange('WF_DLX'); // 初始化 exchange 消费队列，并绑定至 exchange

    await mq.assertQueue('WF_OVERTIME_PRODUCER');
    await mq.bindQueue('WF_OVERTIME_PRODUCER', 'WF_DLX', 'WF-DELAY'); // 初始化所有的延时队列
    ...
    await Promise.map(queues, async singleQueue => {
      ...
      await mq.assertQueue(queueName, {
        ...
        deadLetterExchange: 'WF_DLX',
        deadLetterRoutingKey: 'WF-DELAY'
      });
    });
  }
  ...
  async onDelayMessage() {
    const mq = this._mq;
    mq.consume('WF_OVERTIME_PRODUCER', async content => {
      const {
        target,
        originMsg,
        insertDate,
